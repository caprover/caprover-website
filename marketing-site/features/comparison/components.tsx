...
export function CompareHeader({
  ...
  return (
    <header className="compare-header compare-shell">
      <Logo locale={locale} />
      <div
        className="compare-header-links"
        role="navigation"
      ...
      >
        ...
        <LocaleSwitcher
          locale={locale}
          path={path}
          ariaLabel={getMessages(locale).common.languageSelectorAriaLabel}
        />
      </div>
    </header>
  );
  ...
  return (
    <footer className="compare-footer">
      <div className="compare-shell compare-footer-grid">
        ...
        <small>
          {copy.footer.lastVerified} {copy.verifiedDate}
        </small>
      </div>
    </footer>
  );
...
export function ProofStrip({ locale = DEFAULT_LOCALE }: { locale?: Locale }) {
  const copy = getMessages(locale).comparisonCommon;
  return (
    <div className="proof-strip" aria-label={copy.proofsAriaLabel}>
      {messageList(copy.proofs).map((proof) => (
        <div key={proof.value}>
...
function TableDisclaimer({ locale }: { locale: Locale }) {
  ...
  return (
    <p className="comparison-table-disclaimer">
      {copy.disclaimerBeforeEmail}{" "}
      <a href={`mailto:${copy.disclaimerEmail}`}>{copy.disclaimerEmail}</a>.
    </p>
  );
}
    ...
    <>
      <div className="comparison-table-wrap">
        <table className="comparison-table">
          <thead>
            <tr>
              <th scope="col">{copy.table.capability}</th>
              {data.products.map((product) => (
                <th
                  scope="col"
                  className={product.key === focus ? "is-focus" : ""}
                  key={product.key}
                >
                  {product.label}
                </th>
              ...
              <tr key={row.feature}>
                ...
                {data.products.map((product) => (
                  <td
                    ...
                    key={product.key}
                  >
                    <StatusCell value={row[product.key]} locale={locale} />
                  </td>
                ))}
              </tr>
            ))}
            ...
            {rows.map((row) => (
              <tr key={row.feature}>
                ...
                <td className="is-focus">
                  <StatusCell value={row.caprover} locale={locale} />
                </td>
                <td>
                  <StatusCell value={row.competitor} locale={locale} />
                </td>
              </tr>
            ))}
          </tbody>
...
export function SimplicityPower({
  ...
  return (
    <div className="simplicity-power">
      <article>
        <p className="compare-kicker">{copy.simpleKicker}</p>
        <h2>{copy.simpleTitle}</h2>
        <p>{copy.simpleDescription}</p>
      </article>
      <article>
        <p className="compare-kicker">{copy.powerfulKicker}</p>
        <h2>{copy.powerfulTitle}</h2>
        <p>{copy.powerfulDescription}</p>
      </article>
    </div>
  );
}
...
export function ResourceCallout({
  ...
  return (
    <aside className="resource-callout">
      ...
      <div>
        <p className="compare-kicker">{copy.kicker}</p>
        {children}
  ...
  return (
    <div className="choice-grid">
      <article className="is-caprover-choice">
        <p className="compare-kicker">{copy.caproverKicker}</p>
        <ul>
          {caprover.map((item) => (
            <li key={item}>{item}</li>
          ))}
      ...
      <article>
        <p className="compare-kicker">
          {copy.competitorBeforeName} {competitorName.toUpperCase()}{" "}
          {copy.competitorAfterName}
        </p>
        <ul>
          {competitor.map((item) => (
            <li key={item}>{item}</li>
    ...
    <section className="compare-cta">
      ...
      <div className="compare-cta-links">
        <a
          ...
          href={docsUrl("get-started", locale)}
        >
          {copy.install}
        </a>
        <a className="compare-button" href={docsUrl("one-click-apps", locale)}>
          {copy.oneClickApps}
        </a>
      </div>
    </section>
...
export function SourceLinks({
  ...
  return (
    <div className="source-links">
      ...
      <p>
        {copy.sources.lastVerified} {copy.verifiedDate}.{" "}
...
export function ComparePage({
  ...
  return (
    <main className="compare-page" lang={locale}>
