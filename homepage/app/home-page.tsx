...
function CommunitySupport({ messages }: { messages: HomeMessages["community"] }) {
  return (
      ...
      <div className="community-grid">
        <article className="support-card">
          ...
          <a className="collective-widget" href="https://opencollective.com/caprover" target="_blank" rel="noreferrer noopener">
            <img src="https://opencollective.com/caprover/donate/button@2x.png?color=blue" width="300" height="50" alt={messages.donateAlt} />
          </a>
        </article>
        <article className="subscribe-card">
          ...
          <form action="https://caprover.us19.list-manage.com/subscribe/post?u=c4a2955917a02c0480c9c5677&amp;id=d4a57b767d&amp;f_id=007e42e4f0" method="post" target="_blank" noValidate>
            <label className="sr-only" htmlFor="release-email">{messages.emailLabel}</label>
            <div className="subscribe-row">
              <input id="release-email" type="email" name="EMAIL" placeholder={messages.emailPlaceholder} autoComplete="email" required />
              <button type="submit" name="subscribe">{messages.subscribeButton}</button>
            </div>
    ...
    <main>
      ...
      <section className="hero shell" id="top">
        <div className="hero-copy">
          <p className="eyebrow"><span /> {messages.hero.eyebrow}</p>
          <h1>{messages.hero.titleLine1}<br />{messages.hero.titleLine2}<br className="desktop-break" /> {messages.hero.titleLine3}</h1>
          <p className="hero-text">{messages.hero.description}</p>
          <div className="actions"><a className="primary" href={docs}>{messages.hero.getStarted} <ArrowIcon /></a><a className="secondary" href={LIVE_DEMO}>{messages.hero.liveDemo}</a></div>
          <p className="demo-access">{messages.hero.demoPasswordLabel} <code>captain42</code></p>
          <p className="micro-proof"><span>✓</span> {messages.hero.proofs[0]} <i /> <span>✓</span> {messages.hero.proofs[1]} <i /> <span>✓</span> {messages.hero.proofs[2]}</p>
        </div>
      ...
      <section className="intro shell">
        ...
        <h2>{messages.intro.titleLine1}<br />{messages.intro.titleLine2}</h2>
      ...
      <section className="audience"><div className="shell audience-grid">
        <div><p className="section-kicker">{messages.audience.kicker}</p><h2>{messages.audience.title}</h2></div>
        <div className="quote-code"><code><span>more_of(</span>showResults(getUserList())<span>);</span><br /><em>{messages.audience.codeComment}</em><br />apt-get install libstdc++6</code></div>
        <div className="audience-cards"><article><b>01</b><h3>{messages.audience.simpleTitle}</h3><p>{messages.audience.simpleDescription}</p></article><article><b>02</b><h3>{messages.audience.powerfulTitle}</h3><p>{messages.audience.powerfulDescription}</p></article></div>
      </div></section>
      ...
      <section className="tutorial shell" aria-labelledby="tutorial-title">
        <div className="tutorial-copy">
          ...
          <a className="text-link" href={TUTORIAL} target="_blank" rel="noreferrer noopener">{messages.tutorial.link} <ArrowIcon /></a>
        </div>
        <a className="tutorial-media" href={TUTORIAL} target="_blank" rel="noreferrer noopener" aria-label={messages.tutorial.linkAriaLabel}>
          <img src={asset("/caprover-tutorial.gif")} alt={messages.tutorial.imageAlt} />
        </a>
      </section>
      ...
      <section className="blueprints" aria-labelledby="blueprints-title">
        <div className="shell">
          ...
          <div className="blueprints-grid">
            <figure>
              <div className="diagram-frame"><img src={asset("/caprover-workflow.png")} alt={messages.blueprints.workflowAlt} /></div>
              <figcaption><div><strong>{messages.blueprints.workflowTitle}</strong><span>{messages.blueprints.workflowDescription}</span></div><a href={asset("/caprover-workflow.png")} target="_blank">{messages.blueprints.openDiagram}</a></figcaption>
            </figure>
            <figure>
              <div className="diagram-frame"><img src={asset("/caprover-architecture.png")} alt={messages.blueprints.architectureAlt} /></div>
              <figcaption><div><strong>{messages.blueprints.architectureTitle}</strong><span>{messages.blueprints.architectureDescription}</span></div><a href={asset("/caprover-architecture.png")} target="_blank">{messages.blueprints.openDiagram}</a></figcaption>
            </figure>
          </div>
        </div>
      </section>

      <section className="final-cta shell"><div><Mark /><p className="section-kicker">{messages.finalCta.kicker}</p><h2>{messages.finalCta.title}</h2><p>{messages.finalCta.description}</p><div className="actions"><a className="primary" href={docs}>{messages.finalCta.getStarted} <ArrowIcon /></a><a className="secondary" href={LIVE_DEMO}>{messages.finalCta.liveDemo}</a></div></div></section>

      <footer><div className="shell footer-grid"><a className="logo" href="#top"><Mark /><span>CapRover</span></a><p>{messages.footer.tagline}</p><div><a href={docs}>{messages.footer.docs}</a><a href={GITHUB}>{messages.footer.github}</a><a href={SLACK}>{messages.footer.slack}</a></div><small>{messages.footer.copyright}</small></div></footer>
    </main>
  );
}
